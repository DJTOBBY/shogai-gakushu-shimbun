...
- 発行人名は「山仲　元」。編集長コラムは「編集長　山仲　元」名義

**編集長コラムをAIが起草した場合は、本人の言葉に差し替えることを必ず提案すること。** 実名の媒体であり、文責は本人にある。

### 理事長コラム「認定のその先へ」の扱い（第5号から連載開始）

このコラムだけは署名が「一般財団法人 日本生涯学習協議会　理事長　山仲　元」と、**協議会の肩書きを冠する**。
内容も認定制度・講師育成という協議会の事業に直結するため、読者が「協議会の方針発表」と誤読しうる。

**2026年9月14日、ユーザーの判断で「個人の考察として扱い、免責を厚く付ける」方針が確定した。**
以下を毎回必ず守ること。

1. コラム本文の冒頭に `<p class="opinion-flag">` で次の免責を置く
   > 本欄は筆者個人の見解であり、事実報道ではありません。筆者は一般財団法人日本生涯学習協議会の理事長を務めていますが、本稿は同協議会の公式見解ではなく、また本紙は同協議会の刊行物でもありません。
2. `articles[]` の `editorNote` にも同じ趣旨を書く
3. 12面「本紙について」に、理事長コラムについても同様である旨の一文を残す

**この方針を勝手に緩めないこと。** 将来ユーザーが「協議会の方針として出す」と判断を変えた場合のみ、
免責を弱める（その際はこの記述も更新する）。

### 連載の構成（第5号時点）

| 連載 | 掲載面 | 署名 | 備考 |
|---|---|---|---|
| 編集長コラム「学びの周辺」 | 2面 | 編集長　山仲　元 | 第6回まで発行済み |
| 理事長コラム「認定のその先へ」 | 2面 | 協議会 理事長　山仲　元 | 第5号で創刊。免責は上記 |
| 世界の学びを読む | 10面 | 編集部 | 第5号で創刊。海外の教育動向 |
| 学びの現在地 | 10面 | 生成AI | 随時。第5号は休載 |

**連載の回次は必ず `archive.html` か過去号で確認してから振ること。** ユーザーから受け取る原稿には
前回と同じ回次が書かれていることがある（実際に第4号・第5号で連続して起きた）。既刊と重複する場合は
次の番号に繰り上げ、その旨を報告する。
...
### p2の二重管理を構造的に防ぐ書き方（第4号以降の推奨）

`articles[]` の `body` を手で書くと、HTML側とずれる事故が起きる。**HTML側を正とし、そこから
機械的に抽出して `articles[]` を生成する**と、不一致が原理的に起きない。

```python
# HTML側の col-excerpt から本文を抜き出して articles 用の body 配列にする
def excerpt(aid, body_html):
    m = re.search(r'<div class="col-excerpt">((?:(?!</div>\s*<button)[\s\S])*?)</div>\s*'
                  r'<button type="button" class="read-more" data-article-id="'+aid+'"', body_html)
    return [re.sub(r'<[^>]+>','',p).strip() for p in re.findall(r'<p>([\s\S]*?)</p>', m.group(1))]
```

第4号・第5号はこの方式で作り、検証で「完全一致」を確認している。
